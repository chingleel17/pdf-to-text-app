...
def load_config():
    ...
    if os.path.exists(config_file):
        ...
        config['DEFAULT'] = {'LastPDFPath': '', 'LastOutputDir': os.path.expanduser('~/Downloads')}
...
def select_pdf_file():
    ...
    if pdf_path:
        ...
        # 預設輸出檔案名為輸入檔案名
        default_txt_name = os.path.splitext(os.path.basename(pdf_path))[0] + ".txt"
...
tk.Button(root, text="瀏覽", command=select_output_directory).grid(row=1, column=2, padx=10, pady=10)
...
# 轉換按鈕
tk.Button(root, text="轉換", command=convert_pdf_to_text).grid(row=3, column=0, columnspan=3, pady=20)
